...
\begin{itemize}
    \item Note $\dot{\gamma(t)}\in T_{\gamma(t)}\calM$, so the  \tb{unique (horizontal) lift}  $\dot{\gamma}$ is indeed a curve in $T \mathcal{M}$. So the projection of \tb{geodesic orbit} to $\calM$ is the \tb{geodesic line}!
    \item Compare:
    \begin{itemize}[$\ast$]
...
\end{align*}
implies that \green{hamiltonian vector fields preserve their hamiltonian functions}. In case the the above $H$, we have that \cyan{along orbits of the cogeodesic flow, the Hamiltonian is constant}. Thus, the cogeodesic flow maps the set $$E_\lambda=\left\{(q, p) \in T^* \mathcal{M}: H(q, p)=\lambda\right\}$$
